...
hybrid=HybridSearch(vector,'./storage/doecment.json')

question ='推免生综合评定成绩中，学业成绩和各类奖励总分的权重分别是多少？'

source,chunk_id,content,scores,max_cos = hybrid.query(question, embedding, k=5) #query函数返回与问题最相关的top_k条数据，还有相关性分数，以及对应的source和chunk_id等信息，source和chunk_id可以用来在回答中给出引用，content是实际的文本内容，scores是相似度分数，可以用来判断检索结果的相关程度。
